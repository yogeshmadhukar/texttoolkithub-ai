...
export default function HubLogo({ 
  ...
  return (
    <ContainerComponent 
      ...
      title={editable ? (customLogo ? "Click to change custom logo image" : "Click to upload your image logo") : undefined}
    >
      {editable && (
        <input 
          type="file"
      ...
      {customLogo ? (
        ...
        /* Default brand wrench logo SVG from public/logo.svg */
        <svg 
          viewBox="0 0 512 512" 
          className={`${sizeClasses[size]} shrink-0 block transition-transform duration-200 group-hover/logo:scale-105 rounded-xl`}
          xmlns="http://www.w3.org/2000/svg"
          aria-hidden="true"
        >
      ...
      {editable && (
        <>
          ...
          <span
            className="absolute -bottom-1 -right-1 p-1 bg-indigo-650 rounded-full text-white shadow-md border-2 border-white dark:border-slate-900 opacity-0 group-hover/logo:opacity-100 focus:opacity-100 transition-all duration-200 cursor-pointer hover:bg-indigo-600 hover:scale-110 active:scale-95 z-20 flex items-center justify-center"
          ...
          {customLogo && (
            <button
              ...
              title="Reset logo to default placeholder"
